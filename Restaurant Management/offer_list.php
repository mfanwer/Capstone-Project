...
if (!empty($user_id)) {
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        $sql = "DELETE FROM offer_of_day WHERE id='" . $user_id . "'";
        $result = mysql_query($sql);
        $valueInsert = (int) $result;
        if ($valueInsert > 0) {
            header("location:offer_list.php?status=success");
        } else {
    ...
    <html>
        <head>
            <title>Offer of the day List- Johnny on the Spot</title>
                    ...
                    <div class="col-md-8">
                        ...
                        <form class="form-light mt-20" role="form" method="post" action="menu_list.php">
                            <table class="table_list">
                                ...
                                <?php
                                $i = 0;
                                $sql_contact = "SELECT * FROM offer_of_day WHERE resturent_id = '".$user_id."' ORDER BY resturent_id ASC";
                                ...
                                if (mysql_num_rows($res_contact) > 0) {
                                    while ($row = mysql_fetch_array($res_contact)) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="grid_label"><a href="offer_list.php?id=<?php echo $row ['id']; ?>">Delete</a></td>
                                        </tr>
                                        <?php
                                    }
                                } else {
